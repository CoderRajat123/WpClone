...
import com.example.whatsappcloneproject.helper.MessageAdapter;
import com.example.whatsappcloneproject.helper.Messages;
...
public class ChatActivity extends AppCompatActivity {
    ...
    String saveCurrentTime, saveCurrentDate, checkers="";
    UploadTask uploadTask;

    File Path = new File(Environment.getExternalStorageDirectory().getAbsoluteFile()+"/WhatsappXRec");
    ...
    private void SendMessage() {
        ...
        else
        {
            ...
            String messageReceiverRef="Messages/"+msgRecId+"/"+messageSenderid;
            DatabaseReference userMessageRef=RootRef.child("Messages").child(messageSenderid).child(msgRecId).push();
            ...
            messageBodyDetails.put(messageReceiverRef+"/"+messagePushId,messageTextReady);
            RootRef.updateChildren(messageBodyDetails);
            message.setText("");
        }
    }

    private void SendToProfieActivity() {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode==438 && resultCode==RESULT_OK)
        {
            ...
            if(checkers.equals("pdf") || checkers.equals("docx"))
            {
                ...
                DatabaseReference userMessageRef=RootRef.child("Messages").child(messageSenderid).child(msgRecId).push();
                String messagePushId=userMessageRef.getKey();
                StorageReference filepath=storageReference.child(messagePushId+"."+checkers);
               filepath.putFile(fileuri).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                        ...
                        messageTextReady.put("type",checkers);
                        messageTextReady.put("from",messageSenderid);
                        messageTextReady.put("to",msgRecId);
                        messageTextReady.put("messageID",messagePushId);
                        messageTextReady.put("time",saveCurrentTime);
